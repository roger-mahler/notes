# Visualization stuff to check out

+ [History of Infographics](http://infowetrust.com/scroll/)
+ [DrasticData - Data visualization - Treemapping - Data Projects](http://www.drasticdata.nl/index.htm)
+ [ImageMagick: Command-line Tools: Montage](http://www.imagemagick.org/script/montage.php)
+ [Scaling (resizing) with ffmpeg – FFmpeg](https://trac.ffmpeg.org/wiki/Scaling%20(resizing)%20with%20ffmpeg)
+ [Create a thumbnail image every X seconds of the video – FFmpeg](https://trac.ffmpeg.org/wiki/Create%20a%20thumbnail%20image%20every%20X%20seconds%20of%20the%20video)
+ [SVG Charting Libraries - The Media Temple Blog](http://mediatemple.net/blog/tips/svg-charting-libraries/)
+ [p5.js](https://p5js.org/) | [Github](https://github.com/processing/p5.js)
+ [The infographics of xkcd | halfblog.net](https://halfblog.net/2013/01/22/infographics-of-xkcd/)
+ [VOSviewer - Visualizing scientific landscapes](http://www.vosviewer.com/)
+ [Visual Matters | How Data Visualization Drives Business Impact](http://visualmatters.com/)
+ [Timeline JS3 - Beautifully crafted timelines that are easy, and intuitive to use.](https://timeline.knightlab.com/)
+ [Balsamiq](https://balsamiq.com/wireframes/)
+ [IceAgeMapper: Niche viewer, ice age mapping and geovisualization.](https://github.com/WilliamsPaleoLab/IceAgeMapper)
+ [Lightning | Data Visualization Server](http://lightning-viz.org/)
+ [cambecc/earth: a project to visualize global weather conditions](https://github.com/cambecc/earth)
+ [NICTA/cesium-vr](https://github.com/NICTA/cesium-vr)
+ [Vega](https://github.com/vega)
+ [GraphicsMagick Image Processing System](http://www.graphicsmagick.org/)
+ [openseadragon](http://openseadragon.github.io/#download)
+ [RAWGraphs](http://rawgraphs.io/)
+ [Tufte-LaTeX](https://github.com/Tufte-LaTeX/tufte-latex)
+ [Ant Design - A UI Design Language](https://ant.design/)
+ [Storybook](https://storybook.js.org/)
+ [PyImageSearch](https://www.pyimagesearch.com/)

## Image mining

+ [peterk/pimmer: Exploratory code for PDF image mining](https://github.com/peterk/pimmer)
+ [woden234/image-mining](https://github.com/woden234/image-mining)

## Images / OpenCV

+ [OpenCV library](https://opencv.org/) [opencv/opencv](https://github.com/opencv/opencv)
+ [opencvsharp](https://github.com/shimat/opencvsharp/blob/b5ce5d6ddfaceaf5d926fa2614c5f62d3e45b7c6/test/OpenCvSharp.Tests/img_hash/PHashTest.cs)
+ [OpenCV 101 (2 of 4) - YouTube](https://www.youtube.com/watch?v=QZFx9bXEjQY) [OpenCV 101](https://casis.llnl.gov/seminars/opencv)
+ [OpenCV: Drawing Functions in OpenCV](https://docs.opencv.org/3.1.0/dc/da5/tutorial_py_drawing_functions.html)
+ [Tutorials](https://docs.opencv.org/3.1.0/d6/d00/tutorial_py_root.html) [Tutorials](http://shervinemami.info/openCV.html)
+ [OpenCV: Operations on arrays](https://docs.opencv.org/3.1.0/d2/de8/group__core__array.html#ga85aad4d668c01fbd64825f589e3696d4)
+ [acdha (Chris Adams)](https://github.com/acdha) [acdha/image-mining](https://github.com/acdha/image-mining/)
+ [Stack Overflow](https://stackoverflow.com/questions/50876292/opencv-capture-online-video-for-further-processing-without-downloading-the-file)

# Other

+ [Running before you can walk](https://medium.com/@denz_brade/front-end-dev-running-before-you-can-walk-d78a528915b#.3c0shj16x)
+ [Lev Manovich - home](http://manovich.net/)
+ [Enchanting the Desert | Home](http://enchantingthedesert.com/home/)

# Some saved links

+ [Visualising Twitter Friend Connections Using Gephi: An Example Using the @WiredUK Friends Network | Resources | Data Driven Journalism](http://datadrivenjournalism.net/resources/visualising_twitter_friend_connections_using_gephi)
+ [NetworkX — hvPlot 0.4.0 documentation](https://hvplot.pyviz.org/user_guide/NetworkX.html)
+ [NDEx - The Network Data Exchange - CX Data Model](http://www.home.ndexbio.org/data-model/)
+ [National Resource for Network Biology · GitHub](https://github.com/nrnb)
